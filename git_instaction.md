...
Далее в VSC уже изученными командами создается папка куда можно будет копировать созданный на github.com файл.
Далее находясь в новой папке делаем команду **git clone _адрес файла на github.com_**. Файл добавляется в папку на локальном компьютере.

Есть часть команд которая позволяет работать в программе оболочка VSС:
* cd. - переключение между дерикториями   
* 